...
serve(async (req) => {
  ...
  try {
    ...
    const expectedSecret = Deno.env.get("CREDIT_WEBHOOK_SECRET");
    ...
    // Create new folder in "Kredit" -> "Neu"
    console.log("Creating credit folder:", folderName);
    ...
    // Upload form PDF
    const pdfFileName = payload.pdfContent.name || `Kreditantrag_${customerName.replace(/\s+/g, '_')}_${timestamp}.pdf`;
    ...
    return new Response(
      JSON.stringify({ 
        success: true, 
        folder_id: folder.id,
        message: "Credit case created successfully"
      }),
